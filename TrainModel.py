...
def main():
    ...
    batch_print_callback = LambdaCallback(on_batch_begin=lambda batch, logs:batch)
    epoch_print_callback = LambdaCallback(on_epoch_end=lambda epoch,logs:epoch)
    tensorboard=TensorBoard(log_dir='./Graphfinal2', histogram_freq=0,  
